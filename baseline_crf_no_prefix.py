...
def dialog_feature(dialog, dialog_x, dialog_y):
    ...
    for i in range(0, len(dialog)):
        ...
        '''
        if(i == 0):
            cur_utterance_x.append('False')
            cur_utterance_x.append( 'True')
        else:
            pre_utterance = dialog[i - 1]
            if(pre_utterance.speaker != cur_utterance.speaker): cur_utterance_x.append('True')
            else: cur_utterance_x.append( 'False')
            cur_utterance_x.append( 'False')
        '''
        ...
        for j in range(0, len(cur_utterance.pos)):
            ...
            cur_utterance_x.append(p.token)
