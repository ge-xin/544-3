...
def n_gram_combined_feature(cur_utterance, cur_utterance_x, n):
    '''
    :param cur_utterance: is dialog_i, is the data read in
    :param cur_utterance_x: the feature list, there is no change to y
    :param n: means n gram
    ...
    for i in range(0, len(cur_utterance.pos) - n + 1):
        ...
        for j in range(i, i + n):
            cur = cur_utterance.pos[j]
            n_gram += cur.token.lower() + '-' + cur.pos + ' '
        cur_utterance_x.append(str(n)+'_GRAM:'+n_gram[:len(n_gram) - 1])
...
def dialog_feature(dialog, dialog_x, dialog_y):
    ...
    for i in range(0, len(dialog)):
        ...
        n_gram_combined_feature(cur_utterance, cur_utterance_x, 2)
        '''
...
def learn(input_dir):
    ...
    trainer.train('advanced_crf_model_combined.crfsuite')
...
def classify(test_dir, output_file):
    ...
    tagger.open('advanced_crf_model_combined.crfsuite')
